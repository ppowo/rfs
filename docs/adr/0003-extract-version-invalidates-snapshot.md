...
rfs serves items from a per-source **snapshot** stored in SQLite, not by re-running a Flow on every request. A snapshot is derived data: a pure function of `(upstream HTML, extraction code)`. Until now the only invalidation signal was the HTTP conditional cache (ETag / If-Modified-Since): when the source answered 304 Not Modified, `Poll` left the stored snapshot untouched and skipped `Extract` entirely.

That is correct for unchanged code — identical HTML re-derived by the same parser yields the same items — but it breaks the day the Flow's extraction logic changes. A 304 means "the page bytes are the same," not "the derivation is still valid," so a deployed parser change never re-ran against the identical page, and stale snapshots (e.g. all-decades items still served after the parser was narrowed to the 2020s) survived indefinitely until the upstream page itself changed or the database was cleared by hand.
...
- **Cache the last fetched HTML and re-derive on a version mismatch (rejected)**: on a 304 with a changed version, re-run `Extract` against the cached page bytes instead of refetching. Correct and zero extra traffic, but it requires persisting an HTML blob per source (new storage, new size concerns) for a benefit — one skipped fetch right after each deploy — that is negligible for an hourly single-source poller. Rejected as YAGNI; revisit if a source's page is large or fetched very frequently.
